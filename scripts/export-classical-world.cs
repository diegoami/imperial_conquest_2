...
//     (nationTable.Nations[i].NeighbourMask), never derived or typed by hand. One entry per nation,
//     in DAT nation-table order (nationIds' own order); each entry's own neighbour list is bit
//     order (ascending nation code), matching World.StartingNeighbours' own doc comment.
// ============================================================================================
...
var world = new World(
    ...
    Provenance: ProvenanceMap.Of(
        ...
        ("startingNeighbours", "confirmed: T85's DAT nation-table parse (IC2.Data.SaveNationTable.Parse's NeighbourMask field), DAT nation record +0x2B (IC2.Data.DatLayout.NationNeighbourOffset), 16 words -- dat-neighbour-mask.md §2, \"The mask in the DAT\": symmetric, no self bit, 24 pairs across all 16 rows, matching all 101 local saves. Replaces the geometric derivation NeighbourGeography used before this field existed (dat-neighbour-mask.md §6/§7); that derivation reproduces all 24 of these pairs but adds 6 more (Rome-Greece, Thracia-Bithynia, Thracia-Seleucid, Seleucid-Macedonia, Seleucid-Greece, Ptolemaic-Greece) that this field does not carry.")));
...
internal static class RulesetCorpusMap
{
    public static readonly (string Path, string CorpusId)[] Entries =
    {
        ...
        ("diplomacy.aiOwnDiplomacy.allianceMaxPartnerWars", "diplomacy.aiOwnRule.allianceMaxPartnerWars"),
        ("diplomacy.aiOwnDiplomacy.allianceRollDenominator", "diplomacy.aiOwnRule.allianceRollDenominator"),
        ("diplomacy.aiOwnDiplomacy.busyMobilizationThreshold", "diplomacy.aiOwnRule.busyMobilizationThreshold"),
        ("diplomacy.aiOwnDiplomacy.powerUnityDivisor", "diplomacy.aiOwnRule.powerUnityDivisor"),
        ("diplomacy.aiOwnDiplomacy.powerWealthDivisor", "diplomacy.aiOwnRule.powerWealthDivisor"),
        ("diplomacy.aiOwnDiplomacy.warDeclareRollDenominator", "diplomacy.aiOwnRule.warDeclareRollDenominator"),
        ("diplomacy.aiOwnDiplomacy.warTargetRatioBase", "diplomacy.aiOwnRule.warTargetRatioBase"),
        ("diplomacy.aiOwnDiplomacy.warTargetRatioMultiplier", "diplomacy.aiOwnRule.warTargetRatioMultiplier"),
        ...
        ("diplomacy.offerRollDenominator", "diplomacy.offerRollDenominator"),
