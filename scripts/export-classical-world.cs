...
// loads toy-ruleset.json's own already-reviewed confirmed constants whole and cross-checks them
// against the T04 fixtures corpus; it never reads the DAT at all. The text below says that
// instead, matching what "description" (just above) already says. Naming toy-ruleset.json here is
// correct, not a #299 violation: this is the one key whose job is to describe the file's own
// nature, so FindToyProvenanceMentions below skips it (see that function's own remarks).
const string classicalFaithfulIdProvenance =
    ...
    "task T29's exporter from toy-ruleset.json's own confirmed constants (see 'description' " +
    "above) and cross-checked here against the T04 fixtures corpus; not the small, hand-authored " +
    "ruleset kept for unit tests to load.";
