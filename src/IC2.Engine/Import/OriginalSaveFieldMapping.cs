...
/// </para>
/// <para>
/// <strong>The declared-unmapped set, and its scope.</strong> <see cref="FieldMappingKind.DeclaredUnmapped"/>
/// is reserved for exactly three properties, each the subject of its own narrow user waiver of
/// Done-when 2's "zero unmapped fields":
/// <list type="bullet">
/// <item><c>MercenaryRecord.X</c>/<c>Y</c> — the offer's city-tile position — because
/// <see cref="MercenaryPoolSlot"/> has no position field to hold it (docs/tasks/T21.md, "Mercenary
/// position", decision on
/// <see href="https://github.com/diegoami/imperial_conquest_2/issues/321">#321</see>); the missing
/// hiring-position gate itself is bug
/// <see href="https://github.com/diegoami/imperial_conquest_2/issues/325">#325</see>, not this task's.</item>
/// <item><c>WorldPrefix.Cells</c> — the saved map grid's dynamic, seasonal code-1 overlay (19,639 cells
/// in 63 of 99 corpus saves, proven by a round-2 review's cell-by-cell measurement to be real,
/// save-specific state, not terrain) — because <see cref="GameState"/> has no per-cell field to
/// hold it (docs/tasks/T21.md, "The map grid's overlay", decision on the T21 escalation); modelling the
/// overlay is bug
/// <see href="https://github.com/diegoami/imperial_conquest_2/issues/339">#339</see>, not this task's.
/// The rendering-marker cells in the same property (army/fleet/city tiles) are <em>not</em> part of this
/// gap — they are regenerated from imported positions/owners/sizes — but since <c>Cells</c> is one
/// property, the single worst-case entry has to cover the property as a whole; the split is stated in
/// its own reason.</item>
/// </list>
/// <c>OriginalSaveFieldMappingTests</c> pins the <see cref="FieldMappingKind.DeclaredUnmapped"/> set to
/// exactly these three entries, so a fourth appearing — or one of these three disappearing — fails the
/// test rather than silently widening or narrowing the waiver.
/// </para>
/// </remarks>
public static class OriginalSaveFieldMapping
{
    ...
    {
        // ---- WorldPrefix (round 2, review N-b; reclassified round 3, review R2-B1): the 320x140
        // terrain grid plus the 334 city records.
        new FieldMapping(typeof(WorldPrefix), nameof(WorldPrefix.Cells), FieldMappingKind.DeclaredUnmapped,
            "NOT static: a round-2 review measured all 99 corpus saves cell by cell against the shipped " +
            "World's own decoded terrain and the DAT. Of the cells that differ, 2,047 are the rendering " +
            "markers a live army, a launched fleet or a city leaves on its own tile -- these ARE " +
            "regenerated by the import, from the units' imported positions and the cities' imported " +
            "owners/sizes, so they are not a gap. The remaining 19,639 cells, in 63 of the 99 saves, are " +
            "a dynamic, seasonal overlay: the SAV reads 1 where the World and the DAT both read 0 (sea), " +
            "with no unit, fleet or city there (mean per save: 119 spring, 76 summer, 157 autumn, 758 " +
            "winter; the DAT has no code-1 cell at all). GameState has no field that could hold a " +
            "per-cell overlay, so this part is genuinely, permanently unmapped, not derivable from " +
            "anything else the import carries. Declared unmapped with this reason rather than silently " +
            "dropped or misclassified Derived (a round-2 mistake this replaces: the grid is not " +
            "'identical in every SAV/DAT', and WorldPrefix's own remarks only ever claimed the shared " +
            "*layout*, not shared *content*). The user's decision on 2026-09-24 (T21 escalation, " +
            "docs/tasks/T21.md \"The map grid's overlay\") extends the narrow waiver to this field; " +
            "modelling the overlay is bug #339, after v0.3.0, not this task's."),
