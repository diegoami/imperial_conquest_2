...
2. [x] **`TRecruitMercs`/`TArmyRecruits` — recruitment and mercenary-hire cost formulas.** **Partly done:** confirmed the mercenary offer's troops field is set to the `0xFFFF` sentinel in code on hire (not just observed via save-diffing), confirmed the 100,000-troop army cap as a hard `≥100,001` check, and found `Label` is an index into a name table (not a nation code) — explains why it never matched a `NationCatalog` code. The exact cost formula (shape `troops × priceTable[type] / 1000 × qualityFactor` is visible, values not yet solved) and literal label→name strings remain open. `TArmyRecruits` (city-garrison mobilization, distinct from mercenary hiring) not yet decompiled.
3. [x] **`TPolitics` — tax income formula.** **Done, exact match:** `income = nationTaxBase × taxPercent / 100`; solving Rome's own 15%/20% data points both give `nationTaxBase = 2,440` exactly. `nationTaxBase` itself not yet identified as a named field. Diplomatic reparation formula (`TPolitics_MakePeace`) not yet decompiled — next up.
