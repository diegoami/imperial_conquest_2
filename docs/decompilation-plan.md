...
4. [x] **`TBattleMap`/`ComputerGeneral`/`FinishBattle` — combat loss formula. Structure confirmed, constants partly open:** traced `ComputerGeneral` down through the AI dispatch chain (found `FUN_004381a4` is unit *placement*, not combat, independently reconfirming the 20-units/army cap) and reached the real shooting/melee resolution functions directly via the known `SHOOTS AT`/`ATTACKS` string references. Melee applies losses to **both sides simultaneously** (matches the structure of the one recorded melee exchange), each side's loss a random-roll term scaled by a unit-type effectiveness ratio and capped at 30,000 and at 40% of that side's own troops; also found a `+2`/`−3` per-side adjustment on an unidentified field (candidate morale). See [decompiled-combat-formula-structure.md](reports/decompiled-combat-formula-structure.md). Concrete constants still open (the type-effectiveness table, and the un-decompiled `FUN_0043845c`/`FUN_00438420` helpers), needed to actually simulate the full recorded battle in [battle-observation.md](reports/battle-observation.md) and compare exact numbers, and to explain the uniform ~2.7% whole-army loss from [field-recruitment-uniform-attrition-and-fleet-drift.md](reports/field-recruitment-uniform-attrition-and-fleet-drift.md) (still unclear whether that's the same mechanic as per-engagement combat or a distinct occupation/attrition pass), captured spoils, and the unexplained 9→2 morale swing with no recorded combat.
