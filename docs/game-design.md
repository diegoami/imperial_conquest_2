...
Attacker strength (archers tripled) vs. defender strength (`loyalty × 150 + finishedFortificationPercent × 250 + populationThousands × 200`, `× 5/3` for a capital with loyalty > 59, `× 4/5` if owner ≠ allegiance), with a separate `× 9/10` on the defender at the siege entry point when the attacking army's nation is the city's allegiance (city `+0x14`) **[confirmed: [`investigations/siege-defender-strength.md`](investigations/siege-defender-strength.md), decompiled-city-capture-resolution.md; the `× 9/10` read at instruction level in research `3f6ca09`]**; forced capture changes population/fortification and pulls loyalty toward a 40 floor, defection changes neither and pulls loyalty toward a 65 floor **[confirmed: decompiled-defection-and-siege-attrition.md, galatia-elimination-and-city-resupply-confirmed.md]**. The population and fortification loss comes from the **siege attempt**, not from the transfer. On **every** attempt, win or lose, and in this order, the original does four things. It discards an in-progress fortification order (`fort > 100 → fort % 100`). It erodes **loyalty, fortification and population**, each as `field = max(field × 3/4, min(field × 19/20 + 1, field × def / atk))`, with `def` taken after the `× 9/10`. It floors population at `maxPopulation / 6 + 1`. Then the attacker takes its casualties (the Combat section below). The erosion has no random term. A failed siege costs each field above 20 about 5 %, and a successful one costs between 5 % and 25 % depending on `def / atk`. **The defending city's troops take no casualties** **[confirmed: [`decompiled-defection-and-siege-attrition.md` §"`FUN_0044b27c`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-defection-and-siege-attrition.md#fun_0044b27c-instruction-by-instruction-2026-09-23), research `3f6ca09`; the percentages derived]**. The merged siege path now performs all four city steps (T63, #295, 562e608). A successful capture can cascade into nearby, weakly-defended, low-loyalty cities of the same nation defecting automatically **[confirmed]**; a nation that loses its last city is eliminated (capital sentinel, unity reset) **[confirmed: galatia-elimination-and-city-resupply-confirmed.md]**. This is ready to implement close to verbatim. Siege *resolution* folds into the instant-battle-resolution engine below rather than being a separate code path. It is not resolved the same way as a field battle, though: it has its own casualty ratio, and only the attacker takes casualties.
