...
- Also loyalty/rebellion tied to tax rate, and gradual diplomatic thaw over time **[confirmed, structure]**. The quarterly loyalty draws are now transcribed from code, though not save-checked **[derived: city-population-growth.md]**: at tax < 11 and loyalty < 80, loyalty rises by `Random(4)`; with probability 1 in 3 it falls by `Random(taxRate) / 8`; a non-capital city under 30 rebels. The rebellion's choice of new owner (`FUN_0044c204`) is fully traced **[confirmed: [`decompiled-quarterly-rebellion.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-quarterly-rebellion.md)]**, and T89 implements it. A dead allegiance nation leaves the city in place, because rebirth is T87's. A live allegiance nation takes the city. Otherwise the city goes to the nation of the last live enemy army within 10 tiles, and failing that to the neighbour with the best `cities − 2 × distance to its capital`. Every transfer is a defection. The weather-event effects (`FUN_004511bc`) were never decompiled **[open]**.
...
Attacker strength (archers tripled) vs. defender strength (`loyalty × 150 + finishedFortificationPercent × 250 + populationThousands × 200`, `× 5/3` for a capital with loyalty > 59, `× 4/5` if owner ≠ allegiance), with a separate `× 9/10` on the defender at the siege entry point when the attacking army's nation is the city's allegiance (city `+0x14`) **[confirmed: [`investigations/siege-defender-strength.md`](investigations/siege-defender-strength.md), decompiled-city-capture-resolution.md; the `× 9/10` read at instruction level in research `3f6ca09`]**; forced capture changes population/fortification and sets loyalty to `max(40, min(60, 100 − L′))`, or `min(90, 140 − L′)` for the allegiance nation, where `L′` is the loyalty after the siege's erosion; defection changes neither, and sets loyalty to `min(65, max(50, 100 − L))`, or `min(90, 140 − L)` for the allegiance nation **[confirmed: decompiled-defection-and-siege-attrition.md, galatia-elimination-and-city-resupply-confirmed.md; the formulas from [`decompiled-quarterly-rebellion.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-quarterly-rebellion.md) §5, research `235af11`]**. The population and fortification loss comes from the **siege attempt**, not from the transfer. On **every** attempt, win or lose, and in this order, the original does four things. It discards an in-progress fortification order (`fort > 100 → fort % 100`). It erodes **loyalty, fortification and population**, each as `field = max(field × 3/4, min(field × 19/20 + 1, field × def / atk))`, with `def` taken after the `× 9/10`. It floors population at `maxPopulation / 6 + 1`. Then the attacker takes its casualties (the Combat section below). The erosion has no random term. A failed siege costs each field above 20 about 5 %, and a successful one costs between 5 % and 25 % depending on `def / atk`. **The defending city's troops take no casualties** **[confirmed: [`decompiled-defection-and-siege-attrition.md` §"`FUN_0044b27c`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-defection-and-siege-attrition.md#fun_0044b27c-instruction-by-instruction-2026-09-23), research `3f6ca09`; the percentages derived]**. The merged siege path now performs all four city steps (T63, #295, 562e608). A successful capture can cascade into nearby, weakly-defended, low-loyalty cities of the same nation defecting automatically **[confirmed]**; a forced capture **conquers** the loser well before its last city: when a non-capital capture leaves it fewer than 6 cities, or when its capital falls and is not moved. The capital moves only when unity > 400 and more than 6 cities remain after the capture, and a city more than 10 tiles away scores at least 1. The conquest hands every remaining city to the winner, and the winner takes a copy of the loser's positive treasury. A nation emptied by defection is eliminated without the conquest's effects **[confirmed: galatia-elimination-and-city-resupply-confirmed.md; [`decompiled-elimination-cleanup.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-elimination-cleanup.md) §4, research `43a44a1` and `dcd8fd7`; T86]**. This is ready to implement close to verbatim. Siege *resolution* folds into the instant-battle-resolution engine below rather than being a separate code path. It is not resolved the same way as a field battle, though: it has its own casualty ratio, and only the attacker takes casualties.
...
fleetPower(f) = ships × condition / 10 + (carriedArmy ? siegeStrength(army)/50 : 0)
                then × (1 + random(4)/10)
...
field battle   winner = higher armyPower, ties to the defender
               ...
               with a human side, also only if the winner's army strength < the loser's, the
               draw taken last, and the treaty is only offered: it needs the human's Yes

siege          atk = siegeStrength(army) = Σ(troops, archers × 3) / 80 × armyMorale
               def = defenderStrength(city), × 9/10 if the army's nation is the city's allegiance
               on every attempt, win or lose, before the outcome test:
                 the city's pending fortification order is discarded; loyalty, fortification and
                 population erode (City capture section above); population floored at maxPop / 6 + 1
                 the attacker takes applyLosses(max(1, min(15, def × 6 / atk)))   (with its deletion pass)
               attacker wins only if atk > def (ties to the defender); the city's troops take no casualties

naval battle   winner = higher fleetPower, ties to the defender; loser's fleet (and any army
               aboard it) destroyed; unity ± floor(loserShips / 2)
               winner damage, FUN_0044B4F8(winner, loserPower, winnerPower):
                 r = max(1, loserPower × 100 / winnerPower);  d = r² / 100          (0 … 100)
                 ships −= ships × d / 300;  condition −= condition × d / 300
                 a carried army takes applyLosses(d); if it still exists and d > 70, it also
                 loses unitCount × d / 250 + 1 random units, counted after applyLosses and each
                 removed swap-with-last
```

`applyLosses` is `FUN_0044AE20`. Its second pass, the small-unit deletion, is **[confirmed: decompiled-defection-and-siege-attrition.md, corrected in research `54b85d0`]**. It is a different rule from the tactical rout floor (`/ 25`). The merged resolver ports this deletion pass at every call site (T63, #295, 562e608); it draws one `Random(15)` per unit rather than the original's fixed 20 per slot, a documented departure (T63 Decision 6). The `ratio` arguments at the siege and naval call sites, and the naval `+ 1`, are as the block above gives them **[confirmed at instruction level: [`decompiled-defection-and-siege-attrition.md` §"`FUN_0044b27c`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-defection-and-siege-attrition.md#fun_0044b27c-instruction-by-instruction-2026-09-23) and [`decompiled-diplomacy-peace-terms-and-instant-battles.md` §"`FUN_0044B5D0` and `FUN_0044B4F8`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-diplomacy-peace-terms-and-instant-battles.md#fun_0044b5d0-and-fun_0044b4f8-instruction-by-instruction-2026-09-23), research `3f6ca09`]**. The merged resolver now uses these ratios at both call sites and includes the naval `+ 1` (T63, #295, 562e608).
...
- **Transport**: one army per fleet, capacity `ships × 500` troops. An embarked army leaves the map, moves with the fleet, and is lost if the fleet is lost. It also takes casualties when its fleet wins a battle or weathers a heavy storm (next item).
- **Condition and repair**: condition is a 0–100% multiplier on combat strength, damaged by battle and by storms, and repaired only at one of your own cities at `ships × points / 5` talents, which costs the fleet its remaining moves.
- **Storms**: the fleet tick's storm pass runs on every fleet at sea, with `dmg = max(1, random(100 − condition) / 10)` and its multipliers ([`investigations/thracia-supply-morale.md`](investigations/thracia-supply-morale.md)). A light storm (`dmg < 6`) costs condition only. A heavy one (`dmg ≥ 6`, reached only away from a friendly coast) calls the naval winner-damage function as `FUN_0044B4F8(fleet, 100, dmg + 100)`, so `r = 10000 / (dmg + 100)` and `d = r² / 100`. **`d` falls as `dmg` rises**: 86 at `dmg` 7, 72 at 17, 57 at the winter spike of 30. The fleet loses `ships × d / 300` ships and `condition × d / 300` condition, which is 19–29 % of each. An army aboard takes `applyLosses(d)`, and when `d > 70`, which is every heavy storm except the winter spike, it also loses `unitCount × d / 250 + 1` whole units **[confirmed: [`supply-driven-morale-and-fleet-attrition.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/supply-driven-morale-and-fleet-attrition.md), the 2026-09-23 addition, research `3f6ca09`; the values of `d` derived]**. The merged storm path now computes `r` and `d` this way and applies the carried army's casualties, including the whole-unit loss above `d > 70` (T63, #295, 562e608).
...
- **Alliances and wars are contagious, one step deep**: allying with a nation drags you into its wars; declaring war drags in the target's allies; the cascade goes no further **[confirmed]**.
...
8. **Battle resolution** (the original's instant resolver, all three variants — field, siege, naval — plus the `improved`-ruleset `combat.onDefeat` scatter behaviour for field/naval). *Done when*, with a **fixed seed** and exact assertions: the higher-power side wins and ties go to the defender; the field winner's casualty ratio is `loserPower × 40 / winnerPower`, a besieger's is `max(1, min(15, def × 6 / atk))` on every attempt, and a winning fleet's carried army's is `d` (research `3f6ca09`); absorbed supplies cap at `troops / 100`; every survivor is at least "average"; unity moves ±25 (±`floor(ships/2)` at sea) and clamps at 990; under `classical-faithful` the loser's army/fleet is destroyed outright; under `improved` the loser survives at the mirrored casualty ratio and relocates 2–4 tiles away onto a valid unoccupied tile, its moves zeroed for the turn, with a boxed-in fallback to the destroyed outcome; sieges are unaffected by `combat.onDefeat` regardless of ruleset. *Not* the Rome/Gaul per-type numbers — those came from the tactical path and are out of scope for this resolver by design.
