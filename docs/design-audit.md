...
**Q1 and Q2 have since been answered by the user** and folded into `game-design.md` (see the notes under each). **Q3, Q5, Q6 and Q8 are now also answered**, all the same way, folded into `game-design.md`'s new "Two shipped presets" table. Q4 is folded into that same table rather than left standalone. Q7 is answered independently. **Q9 is now answered too** (from the user's own play experience), leaving only its paid-case bookkeeping as a small evidence gap. **Q10 is new and open** — raised by the rout mechanic found in the September 2026 battle-replay evidence; it touches only the `improved` half of `combat.onDefeat` and blocks nothing.
