...
- **Naval battle** (`FUN_0044B5D0`): `strength = ships × condition / 10 (+ carriedArmySiegeStrength / 50)`, plus a `random(4) × 10%` bonus; higher strength wins, ties to the defender, **the loser's fleet is annihilated**. The winner takes `FUN_0044B4F8(winner, loserStrength, winnerStrength)`: with `r = max(1, loserStrength × 100 / winnerStrength)` and `d = r² / 100` (0…100), it loses `ships × d / 300` ships and `condition × d / 300` condition. Its carried army takes `FUN_0044AE20` with ratio `d`, and when `d > 70` it also loses `unitCount × d / 250 + 1` random units **[confirmed at instruction level: [`decompiled-diplomacy-peace-terms-and-instant-battles.md` §"`FUN_0044B5D0` and `FUN_0044B4F8`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-diplomacy-peace-terms-and-instant-battles.md#fun_0044b5d0-and-fun_0044b4f8-instruction-by-instruction-2026-09-23), research `3f6ca09`, which corrects that report's earlier argument order, its missing `+ 1` and its "army power"]**. Unity moves `± floor(loserShips / 2)`. News: *"`X` sinks fleet of `Y`."*
...
- A siege attempt **wipes the pending order** (`fort > 100 → fort % 100`). It is the first thing every attempt does, before the defender's strength is computed (`0x0044B2A4`), win or lose **[confirmed at instruction level: [`decompiled-defection-and-siege-attrition.md` §"`FUN_0044b27c`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-defection-and-siege-attrition.md#fun_0044b27c-instruction-by-instruction-2026-09-23), research `3f6ca09`]**.
...
`51 … 70` are **this rule's** bounds — the weekly supply tick clamps to them, and they are exactly the five 4-wide tiers the army panel prints via `moraleNames[(v − 51) >> 2]`; a fresh army starts at 59, mid-range. **They are not the field's bounds.** This section previously said they were, and the corpus contradicts it: six records read **72** (`1_cartago_271_spring_11.sav` and `1_cartago_271_summer_1/3/5/7/9.sav`, all army 0, nation 8), found by T44's field sweep and verified independently by its reviewer ([#137](https://github.com/diegoami/imperial_conquest_2/issues/137)). The tick is one writer of `ArmyRecord +14`, not every writer. The battle-time writer is now confirmed. On entry to a **tactical** battle, `FUN_00437de4` adds `+3` to the army of a side whose own nation is **computer-controlled**. The write is persisted, **unclamped** and made once per battle. The human side gets nothing, and the instant resolver writes no `+14` at all ([`battle-quality-promotion-and-morale-array-decompiled.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/battle-quality-promotion-and-morale-array-decompiled.md) §"The morale formula" and [`supply-driven-morale-and-fleet-attrition.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/supply-driven-morale-and-fleet-attrition.md)'s write index, both corrected in research `1762c84`). The write index lists no other writer besides new-army initialisation (59). Whether this `+3` accounts for the Carthage 72s is not established. Two consequences: a **72 would index a sixth tier name** the five-name lookup does not have (no code in this repository performs that lookup, so there is no live bug), and **T21 must not clamp or reject an imported morale outside 51–70**. Note the **2:1 asymmetry** — recovery is half as fast as decay, so starvation costs roughly twice what it takes to undo.
...
The naval side is the same idea on different terms, and must **not** share an implementation. A fleet at sea with **exactly 0** supplies loses **3 of its ~26 moves** and `random(0..1)` **condition** per turn, with no floor and no free regeneration. But starvation is the *smaller* naval mechanic: the same tick runs an unconditional **storm pass** on every at-sea fleet — `damage = max(1, random(100 − condition) / 10)`, doubled away from friendly coast and again in winter — which **escalates as the fleet degrades**, making naval attrition a death spiral that ends at condition < 40 with *"is lost at sea"* and the fleet destroyed outright. A storm with `damage ≥ 6` also costs ships. It calls the naval winner-damage function as `FUN_0044B4F8(fleet, 100, damage + 100)`, so `d = (10000 / (damage + 100))² / 100`, which **falls** as the damage rises (86 at 7, 57 at the winter spike's 30). The fleet loses `d / 300` of its ships and of its condition, and an army aboard takes casualties at ratio `d`, plus `unitCount × d / 250 + 1` whole units when `d > 70` **[confirmed: [`supply-driven-morale-and-fleet-attrition.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/supply-driven-morale-and-fleet-attrition.md), the 2026-09-23 addition, research `3f6ca09`; the merged code now matches this (T63, #295, 562e608)]**. Condition occupies the structural role morale does (`ships × condition / 10` for naval strength against `(troops / 80) × morale` for land), but unlike morale it has no floor and no free recovery, so the naval failure mode is terminal where the land one is merely bad. Both were confirmed empirically: a starved Carthaginian fleet in `1_cartago_271_*.sav` degrades 85 → 48 over nine turns and then vanishes, with the game's own news log in the final save reading *"A fleet belonging to Carthage is lost at sea."*
...
Re-read against their cited reports and found to be as strong as stated: the calendar model (week `+2 mod 12`, season at the 11→1 wrap, year at Winter→Spring, quarterly billing on the season boundary); tax `income = base × rate / 100` with `base = 2,440` solved twice for Rome; ship upkeep `× 3`; recruitment `cost = (troops / 200) × price[type]`; the 100,000-troop army cap; the melee 40% cap `floor(0.4 × defenderTroops) + 1`; the 30,000 cap; the siege strength shape with archers tripled; the loyalty floors (40 forced capture / 65 defection / toward 90 when the allegiant nation recaptures; since corrected: they are clamp-bound formulas, not floors, per `decompiled-quarterly-rebellion.md` §5, implemented by T86); the cascading defection conditions; the nation-elimination cascade; the SAV layout; the news log as a 40-slot ring buffer; the `[open]` tag on the weather-event effect `FUN_004511BC`. The rebellion check `FUN_0044C204` is no longer open: it is fully traced in `decompiled-quarterly-rebellion.md` and implemented by T89.

Siege defender strength, decompiled directly from `FUN_0044A98C` ([`investigations/siege-defender-strength.md`](investigations/siege-defender-strength.md)): `loyalty × 150 + finishedFortificationPercent × 250 + populationThousands × 200`, then `× 5/3` for a capital with loyalty > 59, then `(strength << 2) / 5` — a `× 4/5` that truncates differently from a subtract-20% reading — when the city's owner ≠ its allegiance. [`decompiled-city-capture-resolution.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-city-capture-resolution.md)'s "−20% when owner ≠ allegiance" and the siege entry point `FUN_0044B27C`'s **×9/10 reduction when the *attacking nation* equals the city's allegiance** are **two separate adjustments in two different functions, both applying**. **[confirmed]** The `× 9/10` is now read at instruction level. It tests the army's owner against the city's allegiance word `+0x14` (`0x0044B2CD`–`B2E9`), multiplies before it divides, and the reduced `def` is the value that feeds the city erosion, the attacker's casualty ratio `max(1, min(15, def × 6 / atk))` and the outcome test. The siege inflicts no casualties on the defender's troops **[confirmed: [`decompiled-defection-and-siege-attrition.md` §"`FUN_0044b27c`, instruction by instruction"](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-defection-and-siege-attrition.md#fun_0044b27c-instruction-by-instruction-2026-09-23), research `3f6ca09`]**. Both adjustments are now modelled as the function applies them: T33 renamed the `× 5/3` branch's threshold to loyalty and gated it on the capital predicate ([#46](https://github.com/diegoami/imperial_conquest_2/issues/46)), and replaced the owner ≠ allegiance percentage with the `4/5` numerator and denominator ([#47](https://github.com/diegoami/imperial_conquest_2/issues/47)).
...
What the evidence now says: annihilation is **not** an instant-resolver simplification. The *tactical* path annihilates the loser too — twice, from the same save, 83,348 troops to zero both times, army record deleted — and there is now a mechanism for it. Every rout costs each surviving friendly unit 6 morale and drags anyone below 30 out with it, while handing every enemy +5; once a side starts losing units that spiral finishes it. *(Corrected 2026-09-23: the drag-out is one level deep. A unit the cascade removes starts no cascade of its own and gives no +5, so the spiral advances one rout decision at a time. [`battle-replayed-rout-mechanic-and-combat-constants.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/battle-replayed-rout-mechanic-and-combat-constants.md) consequence 3, research `eb1c886`.)* So "the loser is wiped out" is a **modelled, emergent behaviour of the original at both levels of detail**, not a corner the instant resolver cut.
