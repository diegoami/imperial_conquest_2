...
- **Owns**: `src/IC2.Engine/Ai/**`, `tests/IC2.Engine.Tests/Ai/**`, and — **granted 2026-09-20, narrowly** — `tests/IC2.Engine.Tests/Presentation/GameSessionTests.cs` **for its blanked-field list only** (Done-when 7). No other part of that file, and nothing else under `Presentation/**`.
...
- **Done when**:
  ...
  7. **The two T41 tests this task's behaviour change invalidates are repaired, each repair justified line by line.** *(Added 2026-09-20.)*
      ...
      - **The blanking meta-test.** `GameSessionTests.The_fields_this_test_blanks_really_do_move_with_the_seed` asserts that every field the golden comparison blanks genuinely varies with the seed — it exists precisely so the blank list cannot quietly grow to cover something deterministic. A field this task makes **deterministic** must therefore be **removed from the blank list**, not accommodated by loosening the meta-test. **Narrow the list; never weaken the assertion.** Say in the PR which field stopped varying, and why this task made it so.
- **Hazards**:
  ...
  - **The `GameSessionTests.cs` grant is the blanked-field list and nothing else.** That file is T41's and T23's; you are in it to remove one entry from a list, not to refactor it. If the repair looks like it needs more, **stop and report** — a wider change there belongs to another task and the review will treat it as out of scope.
