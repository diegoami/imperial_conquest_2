...
- **Done when**:
  ...
  16. **Fleet-to-fleet transfer.** `TUnitMap_FleetToFleetTransfer` (`0x004479F4`) opens `TFleetToFleet`, the naval twin of `TArmyToArmy`: a reciprocal ships / supplies / money transfer, committed on OK, with a fleet left at zero ships deleted [`decompiled-unit-map-orders-and-record-fields.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-unit-map-orders-and-record-fields.md). Scope named it all along ("join/split/**transfer**/scuttle"); the first implementation omitted it.
  17. **A fleet can be resupplied, and a fleet can supply others.** The first implementation left `SupplyPurchase.BuyForFleet` callable only from tests while attrition drained `supplies -= ships` every turn from a launch stock of 50 — so every fleet starved within five turns, permanently, for every seat. A command in `Naval/Commands/**` buys supply for a fleet, and **the provider may be a city or another of the nation's own fleets**:
      - **[confirmed]** [`decompiled-unit-map-orders-and-record-fields.md`](https://github.com/diegoami/imperial-conquest-2-research/blob/main/docs/reports/decompiled-unit-map-orders-and-record-fields.md): `TUnitMap_SupplyFleet` (`0x004477FC`) opens `TAFSupply` *"against a city or another fleet"*; `TAFSupply_FindProviders` offers every city within one tile whose owner is not at war with the buyer **and the nation's own fleets within one tile**; the free path's provider stock is *"city `+0x18`, or the fleet's supplies"*.
      - **User testimony (2026-09-18)**, recorded in that report: *"fleet can supply from and to armies, towns and other fleets"* — which settles that the path is reachable in play, not merely present in code.
      - The transfer is `TAFSupply_TransferSupply` (`0x0043FF98`): `target.supplies += amount`, `provider.supplies -= amount`, `treasury[cityOwner] += amount / 5`, `target.money -= amount / 5` — **one talent per five tons, paid from the buyer's own purse to the selling city's owner**, which may be another nation.
      **Call T38's merged `SupplyPurchase.BuyForFleet` and `SupplyCapacity.FleetCapacityTons`; never re-implement the caps or the pricing** (the fleet cap is `ships × 8` with no `+1` dialog bonus, and the purchase is also capped at the buyer's `money × 5` and the provider's stock). The **army-buys-from-a-fleet** direction goes through `BuySupplyCommand`, which is **T38's** Owns list, not this task's — it is filed separately.
