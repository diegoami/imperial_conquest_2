...
- **Owns**: `src/IC2.Engine/Diplomacy/**`, `tests/IC2.Engine.Tests/Diplomacy/**`, `src/IC2.Engine/Model/Ruleset.cs` (**DoD 9's `faithfulThawColumnBug` flag only** — one additive field in the ruleset flags record, nothing else in the file), `data/rulesets/toy-ruleset.json` (the matching key only)
