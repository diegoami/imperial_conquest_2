...
- **Done when**:
  ...
  7. **The two T41 tests this task's behaviour change invalidates are repaired, each repair justified line by line.** *(Added 2026-09-20.)*
      ...
      - **The blanking meta-test.** *(Premise corrected 2026-09-20. The original text assumed a blanked field had become deterministic and told this task to remove it from the list. **That was wrong**, and the implementer refused it after measuring — the hazard working as intended.)*
        ...
        **The repair is to restore the meta-test's statistical power: raise its sweep from 12 seeds to 40** (~25 ms each; 40 suffices for every blanked field here). That is a loop bound, not a weakened assertion — the claim gets **stronger**. Say in the PR what the new sweep costs and why 40.
- **Hazards**:
