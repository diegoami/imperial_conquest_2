...
- The shipped rulesets (`classical-faithful.json`, `improved.json`) are generated by T29, which merges after the last task that changes the schema (T15, T17, T19). A task that changes the schema after T29 has merged also updates T29's export mapping and re-runs it — which makes it `local-only`, and the planner adds that to its entry.
...
- **Sonnet** — 25 tasks. The default for "the design document already says what to build, and the hard part is building it correctly". Correction tasks whose evidence is fully pinned in the entry (T31, T32, T33, T34) sit at Medium; ones that must read a format off decompiled code (T30) or widen the shared domain model (T35) sit at High.
