...
- **Hazards**: the type-effectiveness matrix, the 40% melee cap, the tactical morale array, the per-type shooting-vulnerability weight (unit-type table `+0x20`) and the **rout mechanic** (`FUN_00438fb0` — removal below `standardBattalionSize / 25`, the morale checks, and the −6/+5 morale cascade) are all **research held in reserve** for a possible future detailed resolver — they must not appear in this diff. A reviewer finding them rejects it. The rout mechanic is new since this plan was first written and is the most tempting of the set, because it is the original's *actual* reason a losing army ends at zero troops; resist it anyway — the instant resolver annihilates the loser wholesale and never tracks per-unit attrition, so there is nothing here for it to compute. The `improved` scatter outcome is `[designed, no original analogue]` — its survivor-fraction and scatter-tile-range constants are ruleset data with a documented placeholder default, not a value to hunt for in the decompilation. (`design-audit.md` **Q10** asks whether that placeholder should be re-grounded on the rout thresholds instead; it is **unanswered**, and until it is answered this task ships the placeholder as specified — do not pre-empt it.)
- **One fixtures-corpus correction this task must make first**, under T04's existing `tests/fixtures/**` contract and the same top-up mechanism T08 DoD 13 uses: the corpus entry `battle.tactical.adjacencyPromotionRule` transcribes a rule that has since been **withdrawn upstream** (`battle-quality-promotion-and-morale-array-decompiled.md` now carries a withdrawal banner; see `design-audit.md` §2.10). Mark it withdrawn — or replace it with the uniform 1-in-4 rule already present as `battle.instantResolver.promotionChance` — and fix that sibling entry's `note`, which still describes the adjacency rule as a live second rule on a second code path. It is stale test data rather than a defect in merged code, but it sits directly in this task's subject area and DoD 5 asserts the rule that replaced it, so an implementer who reads the corpus without this correction gets a contradiction. If the orchestrator prefers, this is equally fine as a standalone follow-up issue done before T16 dispatches.
...
- **Done when**:
  1. Three or four representative saves (per `AGENTS.md`'s sampling rule, sample choice justified in the PR body) import, save to the new format, and reload to an equal state. **At least one should be a mid-turn save**: the `0xFFFF` army tombstone T30 taught `IC2.Data` to tolerate has since been caught being created — combat resolution writes it *during* a turn and the end-of-turn tick compacts the record out **[confirmed: battle-replayed-rout-mechanic-and-combat-constants.md]**. So a tombstone is the expected state of a mid-turn save, not an anomaly in three unlucky files, and the importer should be exercised against one deliberately.
