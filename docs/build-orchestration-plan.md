...
> **Keeping the corpus current.** T04 merged with 355 entries drawn from 46 reports, and the research repo has moved on since. The corpus is **topped up by whichever task owns the mechanic the new report describes** — not by reopening T04, whose Owns list (`tests/fixtures/**`) already covers the addition and whose four DoD checks the top-up must keep green. The first instance is the 47th report, `supply-driven-morale-and-fleet-attrition.md`, whose constants are added by **T08** (DoD 13) and consumed by T08 and T14. Adding a report filename to `tests/fixtures/known-reports.json` is part of that top-up, exactly as T04's DoD line 2 anticipates.
...
- **Owns**: `src/IC2.Engine/Model/Ruleset.cs` (the `SiegeRules` record only — the rest of the file, and every other file under `src/IC2.Engine/Model/**`, is untouched), `data/rulesets/toy-ruleset.json`, `tests/IC2.Engine.Tests/Model/**`, `tests/fixtures/**` (the two mis-transcribed corpus entries only — a top-up under §2.4's standing contract, not a reopen of T04), `docs/investigations/siege-defender-strength.md` (new, the write-up)
...
- **Done when**:
  ...
  3. `HighFortificationThreshold` / `HighFortificationBonusNumerator` / `HighFortificationBonusDenominator` are renamed **`HighLoyaltyBonusThreshold` / `HighLoyaltyBonusNumerator` / `HighLoyaltyBonusDenominator`** (values unchanged at 59 / 5 / 3), because the branch tests `DAT_004795a6 > 59` — **loyalty**, not fortification. Their `_provenance` records that the guard the current text calls *"a condition that was not recovered"* **is** recovered: `FUN_0044B8D0`, the is-this-city-its-owner's-capital predicate, identified from the same panel function. No `_provenance` line in the `siege` block still says a condition is unrecovered.
  4. `DefenderOwnerNotAllegiancePenaltyPercent` (20) is replaced by **`DefenderNonAllegiantNumerator` (4) / `DefenderNonAllegiantDenominator` (5)**, because the function computes `(strength << 2) / 5` and a *"subtract 20 percent"* reading truncates differently. A test pins at least one strength value where `s * 4 / 5 != s - (s * 20 / 100)`.
  5. A doc comment on `SiegeRules` states the whole formula as `loyalty × 150 + finishedFortificationPercent × 250 + populationThousands × 200`, and states that the fortification term is the word decoded through **`FortificationCode.FinishedPercent`** — the guarded `code > MaxPercent ? code % radix : code`, matching the function's `if (fort < 0x65) fort else fort % 100` — **not** a raw stored word and **not** an unguarded `% 100`. A test pins `FinishedPercent(100, fortifyRule) == 100` and `FinishedPercent(200, fortifyRule) == 0`, so the difference between the guarded and unguarded decode cannot be lost later. `FortificationCode.cs` itself is unchanged.
  6. The T04 corpus entries **`capture.siegeDefenderStrengthFormula`** and **`capture.fortBonusThreshold`** are corrected in place — same ids, so T04's required-ids and no-duplicate-ids checks stay green — to `loyalty*150 + fortification*250 + population*200` (with the decode noted) and to a **loyalty** threshold with its capital guard named. Both keep `tag: "confirmed"`, and both `note` fields record that the value supersedes `decompiled-city-capture-resolution.md`'s own wording, naming `FUN_0044A98C` and `0x0043BE5C` as the superseding evidence. All four of T04's DoD checks are re-run and green.
  7. The unrelated-but-same-file provenance error the same review found is corrected while the file is open: `combat.naval._provenance.carriedArmyPowerDivisor` currently reads *"a carried army adds armyPower / 50"*; `FUN_0044AA54` calls `FUN_0044A930` (**siege** strength), not `FUN_0044A8CC` (`armyPower`), so it adds *siege* strength / 50. Value unchanged at 50; text corrected. This is the only change outside the `siege` block.
  8. `docs/investigations/siege-defender-strength.md` records the decompilation, the panel-based field identification, the label-to-`DAT_*` table, and a one-line before/after for every field this task renamed or re-weighted — the same shape as `investigations/dat-file-layout.md`, so the evidence is in the repo rather than only in a PR comment.
  9. `dotnet build IC2.sln` and `dotnet test IC2.sln` are green, and `git diff --name-only main...HEAD` lists only Owns paths — in particular **nothing** under `src/IC2.Engine/Strength/**`.
  10. The PR body escalates, without fixing, the three corrections this task deliberately does **not** own: `design-audit.md` §2.13's `[open]` tag on `FUN_0044A98C` (now closed by this evidence, including that the −20%-if-owner≠allegiance and ×9/10-if-attacker==allegiance readings are **two separate adjustments in two different functions**, not one mis-read); the same correction to the research repo's `decompiled-city-capture-resolution.md`; and T17's *"Known-open item to record, not resolve"*, which this settles. Doc edits on `main` are the orchestrator's, per [§2.3](#2-what-makes-the-parallelism-possible).
- **Hazards**: **merge this before T07 resumes and before T16 starts.** T07 is suspended (`status:blocked`, deliberately *not* `status:rework`) on exactly this; its DoD 4 is unsatisfiable while the model's field names are wrong, and its `SiegeStrength.Defender` signature is the first place the swap becomes an executable API with parameter names. T16 consumes siege defender strength through T07 and inherits the same error, so it carries the edge too even though it is waves away. **Do not touch T07's branch, its PR, or `src/IC2.Engine/Strength/**`** — T07 rebases onto this and fixes its own file itself. **Concurrency**: this task and **T08** both write `tests/fixtures/**` (T08 DoD 13's 47th-report top-up), so the orchestrator must not dispatch them together — T31 is small and goes first. Do **not** "fix" the third weight by re-reading `decompiled-city-capture-resolution.md` more carefully: that report is the *source* of the error and re-deriving from it reproduces it. Do **not** touch `AttackerIsAllegianceDefenderReductionPercent` — that ×9/10 lives in `FUN_0044B27C`, is outside this function, and stays T17's to reconcile. Do **not** change any of the five values 150 / 250 / 200 / 59 / 5-over-3: only which field each one multiplies, and which condition gates the bonus, were ever wrong.

#### T30 Harden `IC2.Data`: army tombstones, and the DAT's own file layout

- **Design milestone**: none — closes a **backlog gap**, not a milestone. T01 explicitly rules `IC2.Data`/`IC2.Inspect` out of scope ("do not 'fix' existing code"), and no other task owns `src/IC2.Data/**`; yet **T21 and T29 are both built directly on those parsers**. Nothing in the plan currently owns fixing them. Defect A was surfaced by [`docs/investigations/thracia-supply-morale.md`](investigations/thracia-supply-morale.md), which hit it on real saves; defect B was found while rescoping this task and is written up in [`docs/investigations/dat-file-layout.md`](investigations/dat-file-layout.md). **Labels**: `phase:0 lane:data local-only`
- **Branch**: `task/T30-data-parser-hardening` · **Model/effort**: **Sonnet / High** · **Reviewer**: **Opus / Medium**
- **Start after**: T01 · **Merge after**: T01 — and **merged before T29 starts, and before T21 starts** (see Hazards)
...
- **Scope**: **two distinct defect classes**, both now measured across the whole local corpus (51 `.sav` files plus the DAT), not spot-checked.
  ...
  **B — the DAT is not SAV-shaped, and every table parser after `WorldPrefix` assumes it is. [confirmed]** `SaveNationLayout.Locate` reads an army-count word at `SharedPrefixLength` and walks the army and fleet tables to find the nation table. The DAT has **no count words at all**: its loader `FUN_004481a0` (called by `TPremierForm_NewGame`, `0x0045a9e0`) reads a fixed number of records of each kind and then *assigns* the counts (`DAT_004a0324 = 15`, `DAT_004a0326 = 2`). So on the DAT, `SaveArmyTable` reads army 0's `x` (100) as an army count and throws, and `SaveNationTable`, `SaveFleetTable`, `SaveMercenaryTable`, `SaveRecruitmentTable` and `SaveTurnState` all throw in turn — six of the seven parsers, on a file `WorldPrefix` itself reads perfectly. **The DAT does contain a full 16-record nation table**; it is simply elsewhere, in a shorter record. The complete DAT read order, decompiled from `FUN_004481a0` and accounting for all 140,706 bytes of the file exactly:

  | DAT offset | Bytes | Contents |
  | ---: | ---: | --- |
  | `0x00000` | 89,600 | map — byte-identical in shape to the SAV's (`WorldPrefix.MapByteLength`) |
  | `0x15E00` | 11,356 | city table — same shape as the SAV's (334 × 34) |
  | `0x18A5C` | 15 × 656 | army records, **no count word**; count hardcoded to 15 by the loader |
  | `0x1B0CC` | 2 × 26 | fleet records, **no count word**; count hardcoded to 2 |
  | `0x1B100` | 16 × **1,055** | nation records — the in-memory 1,172-byte record minus the fields the loader never reads |
  | `0x1F2F0` | 1,494 | fourteen fixed static tables, among them the **season table at `0x1F7D8`** (4 × 10 bytes) already cited by `design-audit.md` §2.9a |
  | `0x1F8C6` | 1,040 · 3,012 · 4,992 · 2,440 | four further static tables; the 4,992-byte one is the leader-name pool (16 × 12 × 26 bytes) |

  The 1,172 → 1,055 difference is **not** a constant offset shift, so no amount of adjusting `Locate` will do: the DAT omits the 27-byte leader name at `+0x0b`, the 12 bytes at `+0x424`, three fields the loader *copies* rather than reads (`+0x434`, `+0x43c`, `+0x448`), and the 68-byte trailer `+0x450 … +0x493`. Everything `SaveNationTable` reads **except `Leader` and `HumanPlayer`** is present in the DAT at a known, different offset — treasury `+0x40d`, unity `+0x411`, mobilized `+0x413`, capital `+0x415`, cities `+0x417`, tax `+0x419`. `Leader` and `HumanPlayer` are genuinely absent because `TPremierForm_NewGame` *assigns* them at New Game: the leader by a random draw from the leader-name pool into `record + 0x0b`, and `record + 0x490` to 0. Deliver a DAT-vs-SAV layout discriminator and a DAT-shaped parse path; do **not** teach the SAV locator to "try harder".
- **Done when**:
...
- **Scope**: Tax, quarterly upkeep with real non-payment consequences, tribute growth, loyalty drift and the rebellion check's *confirmed structure* with `_provenance`-tagged placeholder thresholds, the weather-event frequency curve with data-driven effects, **supply as a purchased economy**, **per-turn supply consumption and the supply→army-morale rule it drives**, and per-army/per-fleet money purses gated by the `economy.purses` ruleset flag (`design-audit.md` Q4): `classical-faithful` keeps the confirmed per-army/per-fleet purses (cap 1,000); `improved` routes the same purchases straight to/from the national treasury instead.
  - **Supply consumption and army morale** (`design-audit.md` §2.9a, [`investigations/thracia-supply-morale.md`](investigations/thracia-supply-morale.md), both `[confirmed]`). This subsystem was previously unowned by any task: T07 *consumes* `armyMorale` and T06 owns *when* the tick runs, but nothing owned the rule that writes the field. It lands here because it is a supply rule — the morale write is a function of the supply percentage this task already computes, and of nothing else. **Every constant below is transcribed from that investigation, not re-derived**; an implementer that goes back to the decompilation to rediscover them has misread the task (§2.4).
...
- **Done when** (sharpened from M3, which already names most of these):
  ...
  5. **Revise against `design-audit.md` Q9, now answered by the user's own play experience, not just the code**: resupplying at a city the buying army's nation **owns** is free — no talent debit, matching the confirmed Mediolanum frames (army money and national treasury both unchanged across a 100-ton transfer). A 100-ton purchase at a city the nation does **not** own costs **20** talents, debited from the buying army's purse — the `amount / 5` constant from the code, now scoped correctly to the paid case only. Where a foreign purchase's talents end up (the selling city's owner, or nowhere) is not yet confirmed; implement the debit as specified and treat the credit side as `_provenance`-flagged `[open]` pending more evidence, not as a confirmed destination. This replaces the task's original DoD 5, which applied the debit/credit formula universally and didn't have the free-at-own-city case — a real correction, not a tightening; the implementer should read `design-audit.md` Q9 in full before writing this test.
