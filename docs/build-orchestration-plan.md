...
- **Hazards**: Q9 is now answered (own-city free, foreign-city costs money) — implement DoD 5 as revised and do **not** escalate. The one remaining open detail (where a foreign purchase's talents go) is not a blocker: implement the debit, tag the credit side `[open]` in `_provenance`, and move on.
