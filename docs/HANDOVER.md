...
1. **Start the build** — `docs/game-design.md`'s revised 20-milestone list is ready, and `docs/build-orchestration-plan.md` has broken it into 28 branch-scoped tasks with checkable acceptance criteria, a dependency graph, and a multi-agent implementer/reviewer/orchestrator pipeline driven from GitHub issues (tracker: issue #29). Begin with tasks T01 (build scaffolding and CI), T02 (domain model) and T04 (the fixtures corpus transcribing every exact number from `docs/reports/`). All design decisions that gated the design are made, including the plan's four §11 governance questions (Q-A merge autonomy, Q-B UI visual review, Q-C cost profile, Q-D ruleset-flag deferral — all answered "as recommended").
2. **Q9 is the one remaining open item** in `docs/design-audit.md` §3 — a controlled save around a single supply purchase, to settle whether buying supply actually costs money. Doesn't block milestone 1.
3. **[The release plan](release-plan.md)** is now written — the SemVer scheme, the five release tags (`v0.1.0` … `v1.0.0`) and the merged-task gate for each, who cuts and publishes a tag, the release-note template and its eight required sections, and a 19-line release checklist. Tags land at **capability jumps, not phase boundaries**; each of the 28 task issues now also carries a `release:*` label naming the first release it gates, so a gate is a `gh issue list` query rather than a hand-kept list.
...
5. **New save/screenshot/note cycles still take priority if one shows up** — check `imp_conq_original/notes/2_rome.txt` for anything newer than the last-processed save first.
6. Pick up any "What's still open" item above, per `docs/decompilation-plan.md`'s live status, if `game-design.md` isn't the priority.
