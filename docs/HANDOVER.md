...
1. **Start the build** — `docs/game-design.md`'s revised 20-milestone list is ready to work through with `/loop`, beginning with milestone 1 (data model, loader, and the fixtures file transcribing every exact number from `docs/reports/`). The two decisions that gated it are made: auto-resolve is the original's own instant battle resolver, and naval is in scope.
