...
- **Seven of the nine** open questions in `docs/design-audit.md` §3. Q1 (use the original's own instant battle resolver) and Q2 (naval is in scope) are **answered and folded into `game-design.md`**, whose milestone list has been revised accordingly to 20 milestones with a checkable *done when* on each. Q3–Q9 still need answers, but none of them blocks starting milestone 1.
...
1. **Start the build** — `docs/game-design.md`'s revised 20-milestone list is ready, and `docs/build-orchestration-plan.md` has broken it into 28 branch-scoped tasks with checkable acceptance criteria, a dependency graph, and a multi-agent implementer/reviewer/orchestrator pipeline driven from GitHub issues. Begin with tasks T01 (build scaffolding and CI), T02 (domain model) and T04 (the fixtures corpus transcribing every exact number from `docs/reports/`). The two decisions that gated the design are made: auto-resolve is the original's own instant battle resolver, and naval is in scope. Four governance questions in that plan's §11 are worth answering before the pipeline starts.
2. **Answer `docs/design-audit.md` §3 Q3–Q9 when convenient** — diplomacy fidelity, per-army money purses, default victory condition, human/AI asymmetries, city-development scope, original-bug policy, and one controlled save that would settle whether buying supply actually costs money. None blocks milestone 1.
...
4. **New save/screenshot/note cycles still take priority if one shows up** — check `imp_conq_original/notes/2_rome.txt` for anything newer than the last-processed save first.
5. Pick up any "What's still open" item above, per `docs/decompilation-plan.md`'s live status, if `game-design.md` isn't the priority.
