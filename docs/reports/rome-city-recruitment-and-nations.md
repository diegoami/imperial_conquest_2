...
The screenshot quantities and types match twelve consecutive nonempty 8-byte entries in the save at `0x1A718`–`0x1A777`. Each entry has a first word whose precise meaning is unknown, a troop type code, a troop quantity, and city index **85** (Rome). **Later nation-record analysis corrected the surrounding structure:** these are Rome's 40 recruitment slots at offset `+0x2E4` inside its 1,172-byte nation record, which begins at `0x1A434`. The remaining 28 Rome slots are empty in this save. Each of the other 15 nation records has its own 40-slot recruitment section. The parser currently accepts the observed two-fleet save layout; see the [Ptolemaic player and Week 9 report](ptolemaic-player-and-week9.md) for the record boundaries and controlled turn comparison.
