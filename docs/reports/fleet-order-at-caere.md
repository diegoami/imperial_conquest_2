...
> **Correction (`CityIndex` at `+20`, and the 333/335 question).** The controlled order really does write Caere's city index to `+20` — but only because the fleet is *under construction*. `FUN_0044A050` (construction completes) overwrites `+20` with **`100`**, and from then on it is the fleet's **condition percentage**: `TRepairFleet` displays it as `"N %"`, charges `ships × points / 5` to raise it, and naval combat computes `strength = ships × condition / 10`. That reading explains this report's own data better than the CityIndex one did — the four fleets at `(0,0)` are under construction (so `+20` genuinely is a build-city index, which is why they matched their port's owner 4-for-4 in `fleet-owner-field-confirmed.md`), while the two deployed fleets' `97` and `100` are condition percentages, not Cales and Capua, and the Athens fleet's `166` is a city index rather than an impossible percentage. The `+10` word this report noted as "one word is 24" is the **construction countdown** (24 at order, `0xFFFF` once launched). And the `333`/`335` distinction is closed: markers encode `owner + a size band` (fleets `300/316/332` for `<25 / 25–50 / ≥50` ships), so `333` is Carthage's 90-ship fleet and `335` is Ptolemaic's 70-ship fleet — both independently confirmed by `fleet-owner-field-confirmed.md`'s owner words. See [decompiled-unit-map-orders-and-record-fields.md](decompiled-unit-map-orders-and-record-fields.md). `IC2.Data` still carries the `CityIndex` label.
