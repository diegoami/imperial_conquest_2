...
The observed high codes `333` and `335` are at water cells; for example, `7.sav` has code `333` at `(69, 79)`, where `7.2.png` shows a boat icon. Codes `200`–`299` are on land or river; for example, `7.sav` has code `232` at `(102, 44)` near Rome, where the registered `7.1.png` view shows an army icon. The observed army codes fit `200 + owner code + 16 × variant`: code `232` implies owner `0` (Rome), and adjacent code `222` implies owner `6` (Gaul). The two fleet codes fit `332 + owner code`, giving owner `1` for code `333` and owner `3` for code `335`, consistent with their western and Egyptian locations. These unit-owner formulas are plausible but still need confirmation against their post-city records. The map code is not a unique entity ID; two army cells share code `201` in `7.sav`. Troop counts and orders remain unknown.

The Godot viewer keeps original data outside Git, offers the initial DAT and locally available saves in a source selector, and draws original-coordinate city, army, fleet, and river overlays using new code-native shapes. It does not copy original sprites or screenshots into the repository.

## Next checks

1. Correlate each map marker with the post-city unit records and screenshot detail panels, including the known army near Rome.
2. Verify the meaning of water value `1` and whether a unit can cover a river or city marker in every save.
