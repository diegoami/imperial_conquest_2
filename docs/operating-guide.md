...
- **In progress**: T09 (#9) — movement and terrain.
- **Escalated**: T08 (#8) — economy, supply and purses. PR #53 failed review at `review-round:2` on a supply-capacity source disagreement (the decompiled `troops / 100` cap gives 481 t for the Roman 13-unit army; the corpus and `11_supply.sav` show it at 482 t after a confirmed transfer) — [evidence](https://github.com/diegoami/imperial_conquest_2/issues/8#issuecomment-5662469847). No round 3 is dispatched; T08 is waiting on the planner and the user to pick a resolution (keep the 481 t cap, allow the extra ton, or tag the free-path exemption `[designed]`).
- **Ready**: T10, T11, T12, T33, T34.
- **Next (planner)**: decide T08's escalation (issue #8) with the user. A fresh orchestrator mandate is under way for the remaining scope and has dispatched T09 (#9); T10, T11, T12, T33, T34 remain to dispatch ([build-process.md §5.1](build-process.md#51-who-runs-it)).
