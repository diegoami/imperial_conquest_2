# Release plan: versioning, tags, and release notes for the 28-task build

This document says **when a version number changes, what it is called, who creates it, and what has to be true before it exists**. It is the missing third piece alongside [game-design.md](game-design.md) (*what* is being built — 20 design milestones) and [build-orchestration-plan.md](build-orchestration-plan.md) (*how* it gets built — 28 tasks, 4 phases, a critical path, an implementer/reviewer/orchestrator pipeline over GitHub issues #1–#28).

It **invents no new structure**. Every release gate below is a set of task issues from the orchestration plan's [§12 task index](build-orchestration-plan.md#12-task-index); every human sign-off is one the plan's [§11](build-orchestration-plan.md#11-open-questions-for-the-user) already reserved to the user; the reviewer of a release note is the plan's existing reviewer role, not a new one. Where this document *decides* something, it says so; where it only *recommends*, it says that too.

> **Two different things both called "milestone".** `game-design.md` has **design milestones** M1–M20 (subsystems). The orchestration plan has four **GitHub build-phase milestones** (Phase 0 Foundation, Phase 1 Pure rules, Phase 2 Systems, Phase 3 Delivery). This document adds a third axis — **release versions** — and deliberately does **not** turn them into GitHub milestones; see [§6](#6-what-was-created-in-github-and-what-was-not).

Related reading, in order: [HANDOVER.md](HANDOVER.md) → [game-design.md](game-design.md) → [design-audit.md](design-audit.md) → [build-orchestration-plan.md](build-orchestration-plan.md) → this document. Live pipeline state: [tracking issue #29](https://github.com/diegoami/imperial_conquest_2/issues/29).
...
- **`1.0.0` is crossed by packaging, not by feature count.** The engine is feature-complete at `v0.3.0`; `v1.0.0` is reached when [T27](build-orchestration-plan.md#t27-packaging)'s export launches on a machine with no Godot and no .NET SDK on `PATH` and [T28](build-orchestration-plan.md#t28-nightly-regression-and-soak-gate)'s nightly gate is green. "Playable" means *installable and finishable*, not *implemented*.
- **Pre-1.0, the save format and the ruleset schema may break at any MINOR.** This is safe rather than reckless only because [T20](build-orchestration-plan.md#t20-new-format-saveload-and-versioning)'s DoD requires an unknown *future* save version to be rejected with a typed error and an older one to load through a migration — a broken save fails loudly, never silently best-effort parses. Every release note must state whether the previous tag's saves still load ([§4.2](#42-what-a-release-note-must-contain) item 6).
...
The justification is the orchestration plan's own rule, [§7.2](build-orchestration-plan.md#72-where-the-state-lives): *"This document is the intent. GitHub is the state."* Phase completion **already has a representation** — the phase's GitHub milestone closes and its issues all read `status:merged`. A `v0.0.x-phase0` tag would duplicate state GitHub already holds, on the one axis (git history) the plan deliberately keeps free of progress tracking. Tags are reserved for the thing GitHub milestones *cannot* express: "here is a tree someone can go and use."
...
| --- | --- | --- |
| Phase 0 complete (T01–T05) | **No** | Nothing runs. A placeholder test passing is not a release. |
...
Five releases, anchored to merged task issues, not to dates — this project has no calendar, only dependency order (orchestration plan [§4.1](build-orchestration-plan.md#41-waves-and-the-critical-path)).

| Tag | Gate: all of these `status:merged` | Design milestones complete | What a user can actually do |
| --- | --- | --- | --- |
| **`v0.1.0`** *The rules run* | #1–#12 (T01–T12) — Phase 0 + Phase 1 | M1, M2, M3, M5, M6, M13; M17's buffer | **Nothing a player can do.** A developer clones, runs `dotnet test`, and watches the original's confirmed economy, calendar, movement, strength and victory numbers reproduce from the fixtures corpus. |
...
Cut a `v0.x.y` only when something merged after a tag **fixes a defect that tag shipped** — a wrong constant reaching `main` is the failure mode this project has already hit twice ([§11 Q-C](build-orchestration-plan.md#11-open-questions-for-the-user)), and it deserves its own tag so "which build had the bad number" is answerable. Ordinary forward progress toward the next capability jump is **not** a patch release; it is untagged commits on `main`.

### 2.2 What is explicitly *not* in this ladder
...
| Moving a tag | **Never.** Re-pointing a published tag is a destructive git operation under orchestration plan [§6.6](build-orchestration-plan.md#66-when-to-escalate-to-the-human-instead-of-auto-merging) case 10. A mistake is corrected by a new PATCH tag. |

### 3.2 Who cuts the tag — **RECOMMENDED, consistent with §11 Q-A**

[Q-A](build-orchestration-plan.md#11-open-questions-for-the-user) already granted the orchestrator full merge autonomy **except** the four architecture PRs (T02, T03, T16, T22), and [Q-B](build-orchestration-plan.md#11-open-questions-for-the-user) reserved *visual* judgment to the user. The consistent extension:
...
### 3.3 How this interacts with branch-per-task and squash-merge (§8)

The orchestration plan's [§8](build-orchestration-plan.md#8-git-and-github-conventions) gives `main` a linear history of squash-merges, one per task. Tagging slots into that cleanly:

1. **The tag is cut inside the same `/build-tick` that merges the last gating task**, as a step between the plan's [§7.3](build-orchestration-plan.md#73-one-tick) step 4 (dispatch) and step 5 (post the status table) — *not* as a separate later pass.
2. **Dispatch pauses for that tick.** No other PR is merged between the last gating merge and the tag, so the tagged tree is exactly the tree the release checklist was run against. This costs at most one tick of idle time and removes the entire class of "the tag has a commit nobody tested" bug.
...
4. **A tag is never cut while a task branch is mid-rebase or a conflict is unresolved** (plan §7.4), because merge order is dependency order and a conflicting branch means `main` is about to move for a reason the checklist did not see.
...
A running `CHANGELOG.md` on `main` is rejected for the same reason the orchestration plan [§2.3](build-orchestration-plan.md#2-what-makes-the-parallelism-possible) rejects every other shared registry file: a file every task wants to append to is a guaranteed merge conflict across a dozen in-flight branches, and working around that by making the orchestrator the only writer just moves the same information into a second place that can drift from GitHub. GitHub is already the state.

The note is **generated at cut time** from four sources, in this order:

| Section | Source | Command shape |
| --- | --- | --- |
| What landed | Merged PR titles since the previous tag — already canonical, since §8 mandates `T09 Movement and terrain` | `gh pr list --state merged --base main --json number,title,closingIssuesReferences` |
...
| Known gaps | Design milestones whose tasks are not all `status:merged` | derived from the [§12 task index](build-orchestration-plan.md#12-task-index)'s Design-M column crossed with issue state |

The commit range (`git log <prev-tag>..HEAD --oneline`) is the cross-check, not the source: §8's `T09: <subject>` commit subjects make every commit traceable to a task, so a commit on `main` with no task prefix in a release range is itself a finding.
...
4. **Known gaps vs `game-design.md`** — which of M1–M20 are not complete, each with the issue number that will close it. Plus the still-open evidence items the shipped rulesets are carrying behind a flag: `design-audit.md` Q9 (does buying supply cost money — T08's `supplyPurchaseCostsMoney`), and T17's unreconciled −20%/×9⁄10 siege-strength branches.
5. **Fidelity statement** — which golden fixtures from real play pass (`full-battle-resolution-rome-vs-gaul.md`, `battle-recording-melee-cap-confirmed.md`, `army-to-army-transfer-confirmed.md`, `galatia-elimination-and-city-resupply-confirmed.md`, as each becomes applicable), the determinism guard's status, and from `v0.3.0` the 50-seed AI soak result. This is the section that distinguishes this project from a generic reimplementation and it should read as evidence, not as a claim.
...
8. **Open escalations** — any issue still labelled `status:escalated` or `needs-human` at cut time. A release that silently omits a known escalation is the same failure mode [§6.4](build-orchestration-plan.md#64-the-dod-is-not-negotiable-by-an-agent) exists to prevent.
...
No new role. The orchestration plan already has three roles ([§3.1](build-orchestration-plan.md#31-the-three-roles)) and the review contract in [§6.2](build-orchestration-plan.md#62-what-the-reviewer-checks) transfers to a release note almost unchanged:

| §6.2 gate | Applied to a release note |
| --- | --- |
| 1. DoD independently reproduced | The reviewer re-runs the [§5](#5-release-checklist) checklist commands itself in its own worktree, on the tagged tree. The draft's claims are a convenience, never the proof. |
...
**Model for the seat**: the same rule the catalogue uses for fidelity-critical PRs — **Opus / Medium**, because the failure mode here (a wrong fidelity claim shipped to a player) is the one this project has already paid for twice. The reviewer approves by commenting on the draft Release or on the tracking issue; the orchestrator publishes (or, for `v0.4.0`/`v1.0.0`, hands to the human). This is the same *judge separate from executor* separation [§6.3](build-orchestration-plan.md#63-is-a-third-agent-needed-to-merge) already argues for on merges, for the same audit-trail reason.

---

## 5. Release checklist

The concrete "done when" for cutting a release, in the orchestration plan's style: **one line, one runnable check**, with human sign-off only where a machine genuinely cannot decide. Lines marked *(from vX)* apply only from that tag onward.
...
18. **Human** *(v0.4.0, v1.0.0 only)*: visual sign-off given on every screenshot posted by T24/T25/T27, per [§11 Q-B](build-orchestration-plan.md#11-open-questions-for-the-user).
19. **Human** *(v0.4.0, v1.0.0 only)*: the draft Release is published by the user. For `v0.1.0`–`v0.3.0`, the orchestrator publishes (see [§3.2](#32-who-cuts-the-tag--recommended-consistent-with-11-q-a)).

Items 1–17 are checkable by an agent. Items 18–19 are the only human steps, and neither is new — both are [§11 Q-B](build-orchestration-plan.md#11-open-questions-for-the-user)'s existing answer applied at the release boundary.

**If a check fails**, the release does not get cut and nothing is tagged. Fix forward on `main` and re-run the checklist; a failed checklist is never worked around by weakening a line, for exactly the reason [§6.4](build-orchestration-plan.md#64-the-dod-is-not-negotiable-by-an-agent) gives about DoDs.
...
This makes checklist items 1 and 15 a single `gh issue list` query instead of a hand-maintained list, it is additive and reversible exactly like the 29 labels and 28 issues already created for this project, and it does not disturb anything the orchestrator relies on.

**Deliberately *not* created: GitHub milestones for the version tags.** A GitHub issue carries **exactly one** milestone, and all 28 task issues already carry their **phase** milestone, which orchestration plan [§8](build-orchestration-plan.md#8-git-and-github-conventions) names as a convention. A version milestone could therefore only be empty — permanently 0/0, sitting in the milestone list next to the four meaningful phase ones and inviting exactly the phase-vs-version confusion this document opens by warning about — or it could displace a phase milestone, which would break a documented convention to gain nothing. Labels are multi-valued; milestones are not; the gate is a set, so it is a label. If a future release ever needs its own *new* issues (a `v1.0.1` bugfix batch, say), a milestone for that batch is the right tool at that time.

**Deliberately not created: any GitHub Release.** Nothing is shippable — T01 has not merged. The first Release is drafted when [§2](#2-the-release-ladder)'s `v0.1.0` gate is met.
...
| Published by | The orchestrator for `v0.1.0`–`v0.3.0`; the **human** for `v0.4.0` and `v1.0.0`, inheriting §11 Q-B's visual sign-off |
...
| Reviewed by | The existing reviewer role (Opus / Medium), applying §6.2's gates to the draft Release body |
