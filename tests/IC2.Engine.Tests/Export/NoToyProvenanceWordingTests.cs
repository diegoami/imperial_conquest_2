...
/// <c>toy-ruleset.json</c>'s own confirmed constants, cross-checked against the corpus (not the
/// DAT directly -- R1-B1's own finding). <see cref="FindToyProvenanceMentions"/> therefore skips
/// that one key, mirroring the export script's identical exemption in its own copy of this
/// function, for the identical reason.
/// </para>
/// </remarks>
public class NoToyProvenanceWordingTests
{
    ...
    public void The_root_documents_own_provenance_id_is_exempt()
    {
        const string realExporterOutput = """
            {
              "_provenance": {
                "id": "One of the two shipped presets (docs/game-design.md §'Two shipped presets'). Generated by task T29's exporter from toy-ruleset.json's own confirmed constants (see 'description' above) and cross-checked here against the T04 fixtures corpus; not the small, hand-authored ruleset kept for unit tests to load."
              }
            }
            """;
