...
public sealed class AiSwapTradePartnerCommandTests
{
    ...
    /// <summary>
    /// Rework round 1, N3: no test caught either eliminated check going missing. The poorer partner
    /// having been eliminated since the candidate was proposed (in the same proposal pass, another
    /// action could have caused it) is refused with the shared code, before any state change.
    /// </summary>
    [Fact]
    public void RefusedWhenThePoorerPartnerIsEliminated()
    {
        var ruleset = DiplomacyTestbed.Ruleset;
        var state = DiplomacyTestbed.StateOf(
            DiplomacyTestbed.Nation(Me, "Me"),
            DiplomacyTestbed.Nation(Poorer, "Poorer", taxBase: 100, eliminated: true),
            DiplomacyTestbed.Nation(Richer, "Richer", taxBase: 500));
        state = state with { Relations = state.Relations.WithRelation(Me, Poorer, ruleset.Diplomacy.StateCodes.Trade) };
    ...
    /// <summary>Rework round 1, N3: the mirrored case -- the richer candidate has been eliminated.</summary>
    [Fact]
    public void RefusedWhenTheRicherCandidateIsEliminated()
    {
        var ruleset = DiplomacyTestbed.Ruleset;
        var state = DiplomacyTestbed.StateOf(
            DiplomacyTestbed.Nation(Me, "Me"),
            DiplomacyTestbed.Nation(Poorer, "Poorer", taxBase: 100),
            DiplomacyTestbed.Nation(Richer, "Richer", taxBase: 500, eliminated: true));
        state = state with { Relations = state.Relations.WithRelation(Me, Poorer, ruleset.Diplomacy.StateCodes.Trade) };
    ...
    /// <summary>
    /// Rework round 1, N3: the richer candidate's own cap check at command level -- re-checked at write
    /// time, separately from <see cref="AiOwnDiplomacyRule.FindTradeSwap"/>'s own search-time check.
    /// </summary>
    [Fact]
    public void RefusedWhenTheRicherCandidateIsAlreadyAtTheTradeCap()
    {
        var ruleset = DiplomacyTestbed.Ruleset;
        ...
        for (var i = 0; i < cap; i++)
        {
            state = state with { Relations = state.Relations.WithRelation(Richer, $"o{i}", ruleset.Diplomacy.StateCodes.Trade) };
        }

        var result = DiplomacyTestbed.Dispatcher().Dispatch(state, new AiSwapTradePartnerCommand(Me, Poorer, Richer));

        Assert.True(result.IsRejected);
        Assert.Equal(AiSwapTradePartnerRejections.CandidateCapReached, result.Code);
    }
