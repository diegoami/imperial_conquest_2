...
The first milestone is a C#/.NET parser for the shared world prefix in the original `.DAT` and `.sav` files and the army table in known saves. A Godot map viewer uses that parser to show terrain, cities, and save army rosters. Godot is not required to build or run the command-line inspector.

## Godot map viewer

After setting `assets.local.ini`, open `godot/project.godot` with the Godot .NET edition (currently 4.7.2) and run the project. The viewer reads the original DAT from the configured external directory and draws its 320 × 140 map and 334 cities. It opens in a maximized window near Rome; **Show whole map** returns to the overview. Use the source selector to view the initial world or any `.sav` files in the configured `saves` folder. The mouse wheel zooms toward the cursor, and dragging with the left mouse button moves around the map. In a save, click an army flag to see its troop roster, supply, and money; the `11_supply.sav` Roman army at `(100, 42)` matches the user's 13-unit screenshot. City and fleet clicks show the information currently known for them. Rivers follow their six screenshot-backed blue shapes. City ownership colors and some marker details remain provisional, and the viewer does not yet implement turns or combat. It does not copy original assets into the repository.
